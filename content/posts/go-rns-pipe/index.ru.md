...
Причина простая: я хочу писать сетевые инструменты поверх Reticulum на Go, не тащя в зависимости
Python-рантайм. Так появился [go-rns-pipe](https://github.com/x3ps/go-rns-pipe).
...
Библиотека состоит из пяти компонентов:
...
- **`Interface`** — центральный тип. Оркестрирует чтение/запись, управляет состоянием `online`,
  вызывает коллбэки.
...
Флаг `online` и коллбэки защищены `sync.RWMutex`:

```go
type Interface struct {
    mu      sync.RWMutex // защищает: online, started, onSend, onStatus, cancelFn
...
Для E2E-тестов есть Python-зависимости (`rns`, `pytest`), но они изолированы в `flake.nix`
и не попадают в `go.mod`.
...
Есть E2E через Docker/Podman с реальным rnsd — запускается через `make e2e`.
Это уже проверяет TCP и UDP примеры (`examples/tcp`, `examples/udp`) в живой Reticulum-сети.
...
- Примеры реальных транспортов: TCP (с keepalive tuning под Linux) и UDP
...
[x3ps/go-rns-pipe](https://github.com/x3ps/go-rns-pipe). Лицензия — AGPL-3.0.
